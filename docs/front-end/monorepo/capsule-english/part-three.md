...
在`server`项目中执如下命令，`--no-spec`参数表示不生成测试文件
...
这时交互式指令会询问你想要以什么样的前缀作为公共模块的路径别名，我这里以`@libs`为例

![](../assets/capsule-english/nest-library-create.png)

- 类型接口

`-p`参数表示在哪个模块下创建，`--flat`表示不生成文件夹
